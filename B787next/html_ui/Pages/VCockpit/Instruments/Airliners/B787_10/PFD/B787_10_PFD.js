class B787_10_PFD extends BaseAirliners {
    ...
    get templateID() { return "B787_10_PFD"; }
    get instrumentAlias() { return "AS01B_PFD"; }
...
class B787_10_PFD_Compass extends NavSystemElement {
    ...
    onUpdate(_deltaTime) {
        ...
        if (this.infos) {
            ...
            // SIM ON GROUND  - ground check for use later to toggle TAS off on ground
            var OnGround = SimVar.GetSimVarValue("SIM ON GROUND", "Bool");
            ...
            this.infos.simWindSpeed = SimVar.GetSimVarValue("AMBIENT WIND VELOCITY", "Knots");
...
class B787_10_PFD_CompassInfos extends HTMLElement {
    constructor() {
        ...
        this.construct = () => {
            ...
            this.activeWaypointInfoGroup.setAttribute('id', "ActiveWaypointInfoGroup");
            {
                ...
                this.activeWaypointDistanceToGoText.innerHTML = this.simWaypointDistance.toFixed(1).padStart(2, '0');
    ...
    get simGS() { return Number(this.getAttribute('sim-gs')); }
    set simGS(value) { this.setAttribute('sim-gs', `${value}`); }
    get simTAS() { return Number(this.getAttribute('sim-tas')); }
    set simTAS(value) { this.setAttribute('sim-tas', `${value}`); }
    get simWindDir() { return Number(this.getAttribute('sim-wind-dir')); }
    ...
    set simWaypointETA(value) { this.setAttribute('sim-waypoint-eta', `${value}`); }
    get simWaypointDistance() { return Number(this.getAttribute('sim-waypoint-dist')); }
    set simWaypointDistance(value) { this.setAttribute('sim-waypoint-dist', `${value}`); }
    get simIsAutopilotOn() { return this.hasAttribute('sim-autopilot-on'); }
    set simIsAutopilotOn(value) { if (value) {
        this.setAttribute('sim-autopilot-on', '');
    }
    else {
        this.removeAttribute('sim-autopilot-on');
    } }
    get simAutopilotHeadTrackMode() { return this.getAttribute('sim-autopilot-headtrack-mode'); }
    set simAutopilotHeadTrackMode(value) { this.setAttribute('sim-autopilot-headtrack-mode', `${value}`); }
    get simAutopilotHeadTrackValue() { return Number(this.getAttribute('sim-autopilot-headtrack-value')); }
    set simAutopilotHeadTrackValue(value) { this.setAttribute('sim-autopilot-headtrack-value', `${value}`); }
...
class B787_10_PFD_PlaneInfo extends NavSystemElement {
    init(root) {
        ...
        this.selcal.textContent = "AS-BONEXT";
